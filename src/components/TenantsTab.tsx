...
import { Tenant, Payment, PropertyOwnerSettings, TenantBalanceInfo, PropertyType } from '../types';
...
export default function TenantsTab({
  ...
  const filteredTenants = tenants.filter((tenant) => {
    const matchesSearch =
      ...
      (tenant.propertyType && tenant.propertyType.toLowerCase().includes(searchTerm.toLowerCase())) ||
    ...
    if (filterPropertyType !== 'all') {
      const type = tenant.propertyType || 'Flat';
      if (type !== filterPropertyType) return false;
    }
  ...
  const getPropertyTypeBadge = (propertyType?: PropertyType) => {
    const type = propertyType || 'Flat';
    let style = 'bg-slate-100 text-slate-700 border-slate-200';
    let icon = <Home className="w-3 h-3 text-slate-500" />;

    switch (type) {
      case 'Flat':
        style = 'bg-sky-50 text-sky-800 border-sky-200';
        icon = <Building2 className="w-3 h-3 text-sky-600" />;
        break;
      case 'Duplex':
      ...
      case 'Godown':
        ...
        icon = <Warehouse className="w-3 h-3 text-orange-700" />;
        break;
    }

    return (
      <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-md border ${style}`}>
        {icon}
        {type}
      </span>
    );
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 p-3 bg-white border border-slate-200 rounded-xl shadow-2xs">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-lg text-xs">
            <span className="text-[11px] font-bold text-slate-500 px-1 hidden md:inline">Property:</span>
            <select
              value={filterPropertyType}
              onChange={(e) => setFilterPropertyType(e.target.value)}
              className="px-2 py-1 bg-white border border-slate-200 rounded-md text-slate-800 font-semibold text-xs focus:ring-1 focus:ring-amber-500"
            >
              <option value="all">Sabhi Types (All)</option>
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between gap-2 pb-3 border-b border-slate-100">
                    <div className="flex items-center gap-1.5 truncate">
                      ...
                      {getPropertyTypeBadge(tenant.propertyType)}
                    </div>
